...
* 별도의 카운팅 배열을 운용해서 각 원소가 각각 몇번 나왔는지 세고, 각 인덱스별로 구간합을 구함. 각 구간합은 그 숫자보다 작거나 같은 모든 숫자들의 갯수의 합을 가리킨다.
...
public class CountingSort {
    ...
    public int[] countingSort(int[] arr){
        ...
        for (int i = arr.length-1; i >= 0; i--) {
            sortedArr[ countArr[arr[i]+cali] - 1 ] = arr[i]; //countArr[arr[i]+cali]해당 숫자의 countArr 구간합을 구한다. cali는 음수 때문에 더해주는 보정값이다.
            countArr[arr[i]+cali] = countArr[arr[i]+cali]-1; //다음에 똑같은 숫자가 올 것에 대비해서 구간합을 하나 줄임.
        }
        return sortedArr;
    }
