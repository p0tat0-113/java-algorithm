...
/*
앞의 문제에서 한가지 조건이 더 추가됨.

이번에는 앞의 버전들과 문제 조건이 좀 다르다.
자연수 N과 M이 주어졌을 때, 아래 조건을 만족하는 길이가 M인 수열을 모두 구하는 프로그램을 작성하시오.
- 1부터 N까지 자연수 중에서 M개를 고른 수열
- 같은 수를 여러 번 골라도 된다.
- 고른 수열은 비내림차순이어야 한다. <- 오름차순이여야 한다는 걸 참 어렵게도 얘기한다ㅋㅋ 가지치기 조건을 하나 추가해주기만 하면 될 듯
...
public class Baek_15652_N과M_4 {
    ...
    private static void dfs(int m, int n, ArrayDeque<Integer> currentNums, List<List<Integer>> result) {
        ...
        for (int i = 1; i <= n; i++) {
            if (currentNums.peek() != null && currentNums.peekLast() > i) {
                continue;
            }
