...
public class Leet_147InsertionSortLIst {
    ...
    //과제로 받고 다시 짜본 코드, 근데 똑같은 아이디어를 기반으로 만들다 보니까 거의 비슷한 코드가 나올 수 밖에 없음.
    public ListNode insertionSortList(ListNode head) {
        ListNode tempHead = new ListNode();
        tempHead.next = head;

        ListNode sortedListTailNode = head;//정렬된 리스트의 맨 끝 노드
        ListNode currentNode = tempHead;

        head = head.next;//이제부터 head는 정렬된 부분배열에 새롭게 집어넣을 숫자를 가리킴.
        while (head != null) {
            while (currentNode != sortedListTailNode) {//연산을 할 때 다 currentNode.next로 하는데, currentNode == sortedListTailNode이면 currentNode.next == head가 됨.
                if (head.val < currentNode.next.val) {//참이 되면 head를 이 사이에 삽입을 해야한다.
                    sortedListTailNode.next = head.next;

                    head.next = currentNode.next;
                    currentNode.next = head;
                    break;//이 부분과
                }
                currentNode = currentNode.next;//이 부분이 없어서 숫자 비교를 멈추지 않고 계속 하고 있었음.
            }

            if (head != sortedListTailNode.next) {//새로운 숫자가 정렬된 리스트의 중간에 삽입된 경우
                head = sortedListTailNode.next;
            } else {//새로운 숫자가 정렬된 리스트 중에서 가장 커서 그냥 그대로 있는 경우
                sortedListTailNode = head;
                head = head.next;
            }
