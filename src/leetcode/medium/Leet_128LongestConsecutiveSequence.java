...
public class Leet_128LongestConsecutiveSequence {
    ...
    public int longestConsecutive(int[] nums) {
        ...
        for (int i = 0; i < nums.length; i++) {
            ...
            if (headNum == Integer.MIN_VALUE) {//이미 사용된 숫자라면 패스함.
            ...
            lengthOfSequenceStartedFrom.put(nums[i],1);//lengthOfSequenceStartedFrom에 nums[i]로 시작하는 수열의 길이가 1임을 입력
            int nextNum = headNum + 1;
            nums[i] = Integer.MIN_VALUE;//headNum이 사용되었음을 표시


            while (true) {
                if (!indexMap.containsKey(nextNum)) {//nextNum이 nums에 존재하지 않는 숫자라면 바로 탈출한다.
                    break;
                }
                if (nums[indexMap.get(nextNum)] == Integer.MIN_VALUE) {//nextNum이 사용된 숫자라면 이 숫자를 시작하는 수열이 반드시 있을 것이다. 이 수열의 길이를 가져와서 더하고 탈출한다.
                    lengthOfSequenceStartedFrom.put(headNum, lengthOfSequenceStartedFrom.get(headNum) + lengthOfSequenceStartedFrom.get(nextNum));
                    break;
                }

                nums[indexMap.get(nextNum)] = Integer.MIN_VALUE;//nextNum이 어떤 수열에 사용되었음을 표시
                lengthOfSequenceStartedFrom.put(headNum, lengthOfSequenceStartedFrom.get(headNum) + 1);
                nextNum += 1;
            }
        }
